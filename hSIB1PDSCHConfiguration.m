...
function [pdsch,K_0] = hSIB1PDSCHConfiguration(dci,NSizeBWP,DMRSTypeAPosition,pat)
    ...
    % Configure the PDSCH PRB set associated to the VRB set as specified by
    % the DCI message. If DCI specifies VRB to PRB interleaving, the start
    % of the BWP is 0 and the bundle size is L = 2 RB for SIB1, as defined
    % in TS 38.211 Section 7.3.1.6.
    
    [Lrbs,RBstart] = hDecodeRIV(NSizeBWP,dci.FrequencyDomainResources);
    if dci.VRBToPRBMapping
        prbset = nr5g.internal.pdsch.vrbToPRBInterleaver(NSizeBWP,0,2);
        pdsch.PRBSet = prbset(RBstart + (1:Lrbs));
    else

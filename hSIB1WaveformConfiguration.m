...
function wavegenConfig = hSIB1WaveformConfiguration(config)
    ...
    % Override DCI default configuration with input one if present
    ...
    % Apply power levels for SS blocks, PDCCHs and PDSCHs
    if (~isfield(config,'Power'))
        config.Power = zeros(size(config.TransmittedBlocks));
    end
    wavegenConfig.SSBurst.Power = config.Power;
...
function [carrier,bwp] = getCommonSCSCarrierAndBWPConfig(ssb,minChanBW)
    ...
    % Configure the size of the SCS common carrier to fit CORESET 0 and SSB
    c0 = (csetFreqOffset+10*scsSSB/scsCommon);
    carrier.NSizeGrid = 2*max(c0,csetNRB-c0);
    
    % Configure BWP for control and data (SIB1). The carrier size above
    % ensures the BWP offset is as defined in TS 38.213 Tables 13-1 to
    % 13-10.
...
function ssBurst = getWavegenSSBurstConfig(ssb)
    ...
    % Select PDCCHConfigSIB1 edto configure aspects related to the allocation
    ...
    elseif ssBurst.SubcarrierSpacingCommon < 60 % FR1
...
function pdcch = getWavegenPDCCHConfig(ss,dci,enable)
    ...
    for ch = 1:length(dci)
        ...
        pdcch(ch).DataSource = dcibits;                 % DCI message bits
        pdcch(ch).DataBlockSize = dci.Width;            % DCI message bitwidth
        ...
        % Allocate first candidate of largest aggregation level available
        % in the search space
...
% Copy field values from source to dest structures

    % Expansion of source struct to match number of elements in dest.
    nd = numel(dest);
    if numel(source) == 1
